...
def var_all_case_LA(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    with open('/root/autodl-tmp/dataset/LA/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = ["/root/autodl-tmp/dataset/LA/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
...
def var_all_case_Pancreas(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    with open('/root/autodl-tmp/dataset/Pancreas/test.list', 'r') as f:
            image_list = f.readlines()
    image_list = ["/root/autodl-tmp/dataset/Pancreas/Pancreas_h5/" + item.replace('\n', '') + ".h5" for item in image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
...
def var_all_case_LA_plus(model_l, model_r, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    ...
    with open('/hy-tmp/zx/paper_exp/data/LA/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = ["/hy-tmp/zx/paper_exp/data/LA/2018LA_Seg_Training Set" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
